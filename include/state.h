...
public:
	...
	counts();
	counts(int _m_num_words, int _m_total_num_tables,
			const int_vec& _m_num_tables_by_z,
			const int_vec& _m_word_counts_by_z,
			const vector<int*>& _m_word_counts_by_zw);
	~counts();

	void set_counts(int _m_num_words, int _m_total_num_tables,
			const int_vec& _m_num_tables_by_z,
			const int_vec& _m_word_counts_by_z,
			const vector<int*>& _m_word_counts_by_zw);
};
...
public:
	...
	void iterate_gibbs_state(bool remove, bool permute,
			hdp_hyperparameter* hdp_hyperparam, bool table_sampling = false);
	void iterate_gibbs_state_online(bool remove,
			hdp_hyperparameter* hdp_hyperparam, bool table_sampling = false);

	void sample_tables(doc_state* d_state, double_vec & q, double_vec & f);
	void sample_tables_online(doc_state* d_state, double_vec & q,
			double_vec & f);

	void sample_table_assignment(doc_state* d_state, int t, int* words,
			double_vec & q, double_vec & f);
	void sample_table_assignment_online(doc_state* d_state, int t, int* words,
			double_vec & q, double_vec & f);

	void sample_word_assignment(doc_state* d_state, int i, bool remove,
			double_vec & q, double_vec & f);
	void sample_word_assignment_online(doc_state* d_state, int i, bool remove,
			double_vec & q, double_vec & f);

	void doc_state_update(doc_state* d_state, int i, int update, int k = -1);
	void doc_state_update_online(doc_state* d_state, int i, int update, int k =
			-1);
...
public:
	...
	void iterate_gibbs_state(bool remove, hdp_hyperparameter* hdp_hyperparam,
	...
	void sample_tables(doc_state* d_state, bool not_init, double_vec & q,
			double_vec & f);
	void sample_tables_online(doc_state* d_state, double_vec & q,
			double_vec & f);
	...
	int sample_new_topic_for_new_table_online(
			const vector<double>& likelihood_per_topic_assignment) const;

	double compute_prior_probability_for_topic_sampling(int doc_id,
			int topic_id, bool decrease_table_counts = false) const;
	...
	double compute_next_doc_table_assignment_likelihood(int doc_id,
			int old_topic_id = -1, int new_topic_id = -1,
			bool new_table = false) const;
	double compute_next_table_assignment_likelihood(int doc_id,
			int old_topic_id = -1, int new_topic_id = -1,
			bool new_table = false) const;
	double compute_next_table_assignment_likelihood_for_non_born_topics(
			int doc_id, const vector<int>& updating_num_tables_by_z,
			int old_topic_id = -1, int new_topic_id = -1,
