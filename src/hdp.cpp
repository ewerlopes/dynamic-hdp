#include "hdp.h"
#include <assert.h>

#define VERBOSE true

#define PERMUTE true
#define PERMUTE_LAG 10
#define TABLE_SAMPLING true
#define INF -1e50

hdp::hdp() {
	m_hdp_param = NULL;
	m_state = NULL;
}

hdp::~hdp() {
	m_hdp_param = NULL;

	delete m_state;
	m_state = NULL;
}

void hdp::free_hdp() {
	if (m_hdp_param != NULL) {
		delete m_hdp_param;
	}
	if (m_state != NULL) {
		delete m_state;
	}
}

void hdp::load(char * model_path, int sampler_num) {
	free_hdp();
	m_state = new hdp_state();
	m_state->load_state_ex(model_path, sampler_num);
}

void hdp::save(char * model_path, int sample_num) {
	m_state->save_state_ex(model_path, sample_num);
}

void hdp::setup_state(const hdp_hyperparameter _hdp_param) {
	if (m_hdp_param != NULL) {
		delete m_hdp_param;
		m_hdp_param = NULL;
	}
	m_hdp_param = new hdp_hyperparameter();
	m_hdp_param->copy_parameters(_hdp_param);
}

void hdp::setup_state(const corpus * c, double _eta, double _gamma,
		double _alpha, const hdp_hyperparameter _hdp_param, int vocab_size) {
	free_hdp();

	m_hdp_param = new hdp_hyperparameter();
	m_hdp_param->copy_parameters(_hdp_param);
	m_state = new hdp_state();

	m_state->set_vocab_size(vocab_size);
	m_state->setup_state_from_corpus(c);
	m_state->allocate_initial_space();
	m_state->m_eta = _eta;
	/// set the pointer for the hyper parameters
	assert(m_hdp_param != NULL);
	/// use the means of gamma distribution
	if (_gamma <= 0) {
		m_state->m_gamma = m_hdp_param->m_gamma_a * m_hdp_param->m_gamma_b;
	} else {
		m_state->m_gamma = _gamma;
	}
	if (_alpha <= 0) {
		m_state->m_alpha = m_hdp_param->m_alpha_a * m_hdp_param->m_alpha_b;
	} else {
		m_state->m_alpha = _alpha;
	}
}

void hdp::setup_doc_info_update(const document * doc) {
	m_state->setup_doc_info_from_document(doc);
	m_state->allocate_space_for_online_updates();
}

int hdp::get_size_vocab() const {
	if (m_state != NULL) {
		return m_state->m_size_vocab;
	} else {
		return -1;
	}
}

double hdp::get_eta() const {
	if (m_state != NULL) {
		return m_state->m_eta;
	} else {
		return -1;
	}
}

int hdp::get_topics_number() const {
	if (m_state != NULL) {
		return m_state->m_num_topics;
	} else {
		return -1;
	}
}

int hdp::get_word_topic_counts(int k) const {
	if (m_state != NULL) {
		return m_state->m_word_counts_by_z[k];
	} else {
		return -1;
	}
}

int hdp::get_word_topic_counts(int k, int w) const {
	if (m_state != NULL) {
		return m_state->m_word_counts_by_zw[k][w];
	} else {
		return -1;
	}
}

void hdp::run() {
	m_state->iterate_gibbs_state(false, PERMUTE, m_hdp_param, TABLE_SAMPLING); //init the state

	printf("starting with %d topics \n", m_state->m_num_topics);

	/// time part ...
	time_t start, current;
	time(&start);
	double dif;

	for (int iter = 0; iter < m_hdp_param->m_max_iter; iter++) {
		printf("iter = %05d, ", iter);

		bool permute = PERMUTE && (iter > 0) && (iter % PERMUTE_LAG == 0);

		m_state->iterate_gibbs_state(true, permute, m_hdp_param,
				TABLE_SAMPLING);

		time(&current);
		dif = difftime(current, start);

		printf("#topics = %04d, time = %8.2f\n", m_state->m_num_topics, dif);

	}
}

void hdp::run_online() {
	m_state->iterate_gibbs_state_online(false, m_hdp_param, TABLE_SAMPLING); //init the state

	for (int iter = 0; iter < m_hdp_param->m_max_iter; iter++) {
		m_state->iterate_gibbs_state_online(true, m_hdp_param, TABLE_SAMPLING);
	}
}

void hdp::free_online_info() {
	m_state->free_online_info();
}
