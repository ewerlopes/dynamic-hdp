...
void run_train_batch_whole_process(int m_samplers, int vocab_size,
		double gamma_a, double gamma_b, double alpha_a, double alpha_b,
	...
	hdp_hyperparam->setup_parameters(gamma_a, gamma_b, alpha_a, alpha_b,
			max_iter);
	...
	for (int s = 0; s < m_samplers; ++s) {
		...
		hdp_instance[s]->setup_state(c, eta, gamma, alpha, *hdp_hyperparam,
				vocab_size);
		...
		if (save_topic_distribution) {
			...
			print_matrix(hdp_instance[s]->m_state->m_word_counts_by_zd,
					hdp_instance[s]->m_state->m_num_docs, file_name);
		}
	}
...
void run_online_process(int m_samplers, int vocab_size, double gamma_a,
		double gamma_b, double alpha_a, double alpha_b, double eta,
		double gamma, double alpha, int max_iter, const char * directory,
		const char * data_path_batch, const char * data_path_online,
		char * model_path, bool hm_likelihood_computing,
		bool save_topic_distribution, int _m_online_documents) {
	...
	while (!eof_online) {
		...
		if (doc->length > 0) {
			for (int s = 0; s < m_samplers; ++s) {
				printf("online doc = %05d, sampler = %05d, ", m_num_online_docs,
						s);

				if (hm_likelihood_computing) {
					counts_before_update[s]->set_counts(
							hdp_instance[s]->m_state->m_size_vocab,
				...
				if (save_topic_distribution) {
					...
					file_name = add_postfix(file_name, s);
					append_matrix(
							hdp_instance[s]->m_state->m_word_counts_by_zd_online,
							file_name);
				}
			}
		}
		...
		if (hm_likelihood_computing) {
			...
			if (doc->length > 0) {
				hm_likelihood =
						l_computer.compute_harmonic_mean_predictive_likelihood(
								hdp_instance, counts_before_update, m_samplers);
			}
			...
			if (hm_likelihood_vec.size() >= m_num_online_docs) {
				hm_likelihood_vec[m_num_online_docs - 1] = hm_likelihood
						/ doc->length;
			} else {
...
void run_train_dynamic_batch_whole_process(int m_samplers, int vocab_size,
		double gamma_a, double gamma_b, double alpha_a, double alpha_b,
		double eta, double gamma, double alpha, double delta, int max_iter,
		const char * directory, const char * data_path, char * model_path,
	...
	for (int s = 0; s < m_samplers; ++s) {
		printf("batch run for sampler = %05d\n", s);
		hdp_instance[s]->setup_state(c, eta, gamma, alpha, delta,
...
void run_online_dynamic_process(int m_samplers, int vocab_size, double gamma_a,
		double gamma_b, double alpha_a, double alpha_b, double eta,
		double gamma, double alpha, double delta, int max_iter,
		const char * directory, const char * data_path_batch,
		const char * data_path_online, char * model_path,
		bool hm_likelihood_computing, bool save_topic_distribution,
	...
	while (!eof_online) {
		...
		if (doc->length > 0) {
			for (int s = 0; s < m_samplers; ++s) {
				printf("online doc = %05d, sampler = %05d, ",
						m_num_online_docs - 1, s);
