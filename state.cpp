...
void hdp_hyperparameter::copy_parameters(const hdp_hyperparameter _input_hyperparam)
{
...
counts::counts(int _m_num_words, int _m_total_num_tables, const int_vec& _m_num_tables_by_z, const int_vec& _m_word_counts_by_z,
               const vector<int*>& _m_word_counts_by_zw)
{
    ...
    for (int k = 0; k < m_num_topics; ++k) {
		...
		copy(_m_word_counts_by_zw[k], _m_word_counts_by_zw[k] + m_num_words, m_word_counts_by_zw[k]);
...
void counts::set_counts(int _m_num_words, int _m_total_num_tables, const int_vec& _m_num_tables_by_z,
						const int_vec& _m_word_counts_by_z,
						const vector<int*>& _m_word_counts_by_zw)
{
	if ((m_num_words == _m_num_words) && (m_num_tables_by_z.size() == _m_num_tables_by_z.size())) {
...
void hdp_state::iterate_gibbs_state_online(bool remove, hdp_hyperparameter* hdp_hyperparam,
                                           bool table_sampling)
{
    double_vec q;
    double_vec f;
    ...
    if (table_sampling) sample_tables_online(d_state, q, f);
    ...
    for (t = 0, new_t = 0; t < num_tables_old; t++)
    {
        if (d_state->m_word_counts_by_t[t] > 0)
        {
			...
			if (k >= m_num_topics_before_update) {
				d_state->m_table_to_topic[new_t] = k_to_new_k[k - m_num_topics_before_update];
            } else {
...
void hdp_state::compact_hdp_state_online()
{
    ...
    int k, new_k;
	for (k = m_num_topics_before_update, new_k = m_num_topics_before_update; k < num_topics_old; k++)
    {
        if (m_word_counts_by_z[k] > 0)
        {
...
void hdp_state::sample_tables_online(doc_state* d_state, double_vec & q, double_vec & f)
{
...
void hdp_state::sample_table_assignment_online(doc_state* d_state,
...
void hdp_state::sample_word_assignment_online(doc_state* d_state, int i, bool remove,
                                              double_vec & q, double_vec & f)
{
    if (remove) doc_state_update_online(d_state, i, -1);
...
// k is only provided when m_table_to_topic doesn't have that
void hdp_state::doc_state_update_online(doc_state* d_state, int i, int update, int k)
{
    int d, w, t;
    d = d_state->m_doc_id;
    w = d_state->m_words[i].m_word_index;
											...
											bool table_sampling)
{
    ...
    for (int j = 0; j < m_num_docs; j++)
    {
        ...
        if (table_sampling) sample_tables(d_state, not_init, q, f);

    }
    compact_hdp_state();
}

void hdp_state_dynamic::iterate_gibbs_state_online(bool remove, hdp_hyperparameter* hdp_hyperparam,
												   bool table_sampling)
{
    double_vec q;
    double_vec f;
...
void hdp_state_dynamic::reset_counts_up_to_date(int doc_id)
{
	if (m_total_num_tables_by_d.size() > 0) {
        if (doc_id == 0) {
            ...
            for (int k = m_num_tables_by_z.size(); k < m_num_tables_by_zd.size(); ++k) {
                assert(m_num_tables_by_zd[k] != NULL);
...
void hdp_state_dynamic::sample_tables(doc_state* d_state, bool not_init, double_vec & q, double_vec & f)
{
    vector <int*> words_by_t;
    words_by_t.resize(d_state->m_num_tables, NULL);
    int* p = NULL;
...
void hdp_state_dynamic::sample_tables_online(doc_state* d_state, double_vec & q, double_vec & f)
{
    vector <int*> words_by_t;
    words_by_t.resize(d_state->m_num_tables, NULL);
    int* p = NULL;
...
void hdp_state_dynamic::sample_table_assignment(doc_state* d_state, int t, bool not_init,
												int * words,
												double_vec & q, double_vec & f)
{
    //number of tables won't change at all
    ...
    double new_topic_prior = compute_prior_probability_for_topic_sampling(d_state->m_doc_id, m_num_topics, false);
    ...
    if ((not_init) && (topic_prior_probability[m_num_topics] > EPS)) {
        next_topic_new = compute_next_table_assignment_likelihood(d_state->m_doc_id, k_old, -1, false);
    }
    ...
    for (k = 0; k < m_num_topics; k ++)
    {
        if (k == k_old)
        {
            ...
            double topic_prior = compute_prior_probability_for_topic_sampling(d_state->m_doc_id, k, true);
            topic_prior_probability[k] = topic_prior;
            
            double next_topic = 0;
            if ((not_init) && (topic_prior_probability[k] > EPS)) {
                next_topic = compute_next_table_assignment_likelihood(d_state->m_doc_id, k_old, k, false);
            }
        ...
        else
        {
            ...
            double topic_prior = compute_prior_probability_for_topic_sampling(d_state->m_doc_id, k, false);
            topic_prior_probability[k] = topic_prior;
    ...
    if (not_init) {
        log_normalize_non_zero_values(next_topics_probability, m_num_topics + 1);
    }
    for (k = 0; k < m_num_topics; ++k) {
        if ((k != k_old) || (topic_prior_probability[k] > EPS)) {
            q[k] = log(topic_prior_probability[k]) + next_topics_probability[k] + f[k];
        } else {
            q[k] = INF;
        }
    }
    q[m_num_topics] = log(topic_prior_probability[m_num_topics]) +
                        next_topics_probability[m_num_topics] +
                        f_new;
...
double hdp_state_dynamic::compute_next_doc_table_assignment_likelihood(int doc_id, int old_topic_id, int new_topic_id, bool new_table) const
{
    ...
    double likelihood = compute_next_table_assignment_likelihood_for_non_born_topics(doc_id,
                                                                               m_num_tables_by_z,
                                                                               old_topic_id, new_topic_id, true);

	return likelihood;
}

double hdp_state_dynamic::compute_next_table_assignment_likelihood_for_non_born_topics(int doc_id,
                                                                const vector<int>& updating_num_table_by_z,
        ...
        for (int k = 0; k < m_num_topics; ++k) {
            if (updating_num_table_by_z[k] > 0) {
                for (int n = 0; n < m_num_tables_by_zd[k][doc_id + 1]; ++n) {
                    likelihood += log(m_num_tables_by_zd[k][doc_id] + n +
                                      m_delta * (updating_num_table_by_z[k] + n));
                }
            }
        }
    } else {
        for (int k = 0; k < m_num_topics; ++k) {
            if (k == old_topic_id) {
                if (updating_num_table_by_z[k] - 1 > 0) {
                    ...
                    for (int n = 0; n < m_num_tables_by_zd[k][doc_id + 1]; ++n) {
                        likelihood += log(m_tables_in_current_doc + n +
                                          m_delta * (updating_num_table_by_z[k] - 1 + n));
                    }
                }
...
double hdp_state_dynamic::compute_next_table_assignment_likelihood(int doc_id, int old_topic_id,
                                                                   int new_topic_id, bool new_table) const
{
    if (doc_id == m_num_docs - 1) {
        return 0; // log(0) = 1
    }
    double next_doc_probability = compute_next_doc_table_assignment_likelihood(doc_id, old_topic_id,
                                                                               new_topic_id, new_table);
													   ...
													   int t, int* words, double_vec & q, double_vec & f)
{
	...
	double topic_prior_new = compute_prior_probability_for_topic_sampling_online(m_num_topics, false);
    ...
    for (k = 0; k < m_num_topics; k ++)
    {
        if (k == k_old)
        {
			...
			double topic_prior = compute_prior_probability_for_topic_sampling_online(k, true);
        ...
        else
        {
            ...
            double topic_prior = compute_prior_probability_for_topic_sampling_online(k, false);
...
void hdp_state_dynamic::sample_word_assignment(doc_state* d_state, int i, bool not_init, double_vec & q, double_vec & f)
{
    if (not_init) doc_state_update(d_state, i, -1);
    ...
    for (k = 0; k < m_num_topics; k++)
    {
        ...
        if ((not_init) && (prior_topic_probability[k] > EPS)) {
            next_topics_probability = compute_next_table_assignment_likelihood(d_state->m_doc_id,
                                                                                  -1, k, true);
        }
        next_doc_topic_assignment_probability[k] = next_topics_probability;
    }
    
    double topic_new_prior = compute_prior_probability_for_topic_sampling(d_state->m_doc_id, m_num_topics, false);
    ...
    if ((not_init) && (prior_topic_probability[m_num_topics] > EPS)) {
        next_topics_probability_new = compute_next_table_assignment_likelihood(d_state->m_doc_id, -1, -1, true);
    }
    next_doc_topic_assignment_probability[m_num_topics] = next_topics_probability_new;
    
    if (not_init) {
        log_normalize_non_zero_values(next_doc_topic_assignment_probability, m_num_topics + 1);
    }
...
int hdp_state_dynamic::sample_new_topic_for_new_table_online(const vector<double>& likelihood_per_topic_assignment) const
{
	vector<double> q;
    ...
    for (int k = 0; k < m_num_topics; k ++)
    {
		double prior_probability = compute_prior_probability_for_topic_sampling_online(k, false);
		
        total_q += prior_probability * likelihood_per_topic_assignment[k];
        q[k] = total_q;
    }
 
	double prior_probability = compute_prior_probability_for_topic_sampling_online(m_num_topics, false);
	
	total_q += prior_probability * likelihood_per_topic_assignment[m_num_topics];
    q[m_num_topics] = total_q;
    double u = runiform() * total_q;
	int new_topic = -1;
...
double hdp_state_dynamic::compute_prior_probability_for_topic_sampling(int doc_id, int topic_id,
                                                                       bool decrease_table_counts) const
{
...
double hdp_state_dynamic::compute_prior_probability_for_topic_sampling_online(int topic_id,
                                                                              bool decrease_table_counts) const
{
    assert(topic_id <= m_num_topics);
    if (topic_id == m_num_topics) {
    ...
    int num_tables_from_prev_doc = 0;
    int actual_size = min((int)m_num_tables_by_zd_for_prev_doc_online.size(), m_num_topics);
    if (topic_id < actual_size) {
        num_tables_from_prev_doc = m_num_tables_by_zd_for_prev_doc_online[topic_id];
    }
...
void hdp_state_dynamic::sample_word_assignment_online(doc_state* d_state, int i, bool init,
													  double_vec & q, double_vec & f)
{
    if (init) doc_state_update_online(d_state, i, -1);
    ...
    int k, t, w;
    w = d_state->m_words[i].m_word_index;
	double topic_new_prior = compute_prior_probability_for_topic_sampling_online(m_num_topics, false);
	...
	double prior_topic_normalisation = compute_normalisation_constant_for_topic_prior_online();
...
// k is only provided when m_table_to_topic doesn't have that
void hdp_state_dynamic::doc_state_update(doc_state* d_state, int i, int update, int k)
{
    int d, w, t;
    d = d_state->m_doc_id;
    w = d_state->m_words[i].m_word_index;
...
// k is only provided when m_table_to_topic doesn't have that
void hdp_state_dynamic::doc_state_update_online(doc_state* d_state, int i, int update, int k)
{
    int d, w, t;
    d = d_state->m_doc_id;
    w = d_state->m_words[i].m_word_index;
...
void hdp_state_dynamic::compact_doc_state_online(doc_state* d_state, int* k_to_new_k)
{
    int num_tables_old = d_state->m_num_tables;
    int* t_to_new_t = new int[num_tables_old];
...
void hdp_state_dynamic::load_state_from_file(char * name)
{
    ...
    for(int k = 0; k < m_num_topics; k ++)
    {
        fread(&(m_num_tables_by_zd_for_prev_doc_online[k]), sizeof(int), 1, file);
		fread(&(m_num_tables_by_z[k]), sizeof(int), 1, file);
        fread(&(m_word_counts_by_z[k]), sizeof(int), 1, file);
...
void hdp_state_dynamic::save_online_update_for_next_iteration()
{
    ...
    m_total_num_tables_by_d_for_prev_doc_online = m_total_num_tables_by_d_online;
}
...
void hdp_state_dynamic::save_last_document_info_for_online_update()
{
    ...
    for (int k = 0; k < m_num_topics; ++k) {
        m_num_tables_by_zd_for_prev_doc_online[k] = m_num_tables_by_zd[k][m_num_docs - 1];
    }
    
    m_total_num_tables_by_d_for_prev_doc_online = m_total_num_tables_by_d[m_num_docs - 1];
}
